...
def getModel_FCN():
    ...
    mypotim=Adam(lr=0.0003, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
...
#
#Xtrain = autoencoder.predict(Xtrain)
...
model.fit(Xtr_more[:,:,:,0:], Ytr_more, batch_size=batch_size, epochs=30, verbose=1, callbacks=[earlyStopping, mcp_save,tensorboard,reduce_lr_loss], validation_split=0.25)
...
Xtest0 = Xtest[idx_tr[0],...]

#Run a DAE
#autoencoder = dae()
#autoencoder.summary()
#
#autoencoder.fit(Xtest, Xtest, epochs=5, batch_size=32, shuffle=True, validation_split=0.25)
#
...
submission.to_csv(INPUT_PATH + '20171127_submission_3.csv', index=False)
### ----
#predA_test0 = model.predict(Xtest0[:,:,:,0:])
...
submission.to_csv(INPUT_PATH + '20171127_submission_3_pl.csv', index=False)
